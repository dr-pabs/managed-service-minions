# Agent Definitions

This folder contains agent-role definitions for the Minions Agent Framework delivery model.

## Files
- [framework-engineer-agent.md](framework-engineer-agent.md)
- [integration-engineer-agent.md](integration-engineer-agent.md)
- [infrastructure-engineer-agent.md](infrastructure-engineer-agent.md)
- [prompt-engineer-agent.md](prompt-engineer-agent.md)
- [frontend-engineer-agent.md](frontend-engineer-agent.md)
- [security-auditor-agent.md](security-auditor-agent.md)
- [test-engineer-agent.md](test-engineer-agent.md)
- [devops-agent.md](devops-agent.md)
- [product-analyst-agent.md](product-analyst-agent.md)

These definitions align with the agent roles described in [../docs/agent-led-development.md](../docs/agent-led-development.md).
