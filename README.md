...
| **Item pipelines** | Declarative classify→act→verify→commit/escalate chains with per-item verification, wired into the production consumer's per-`item_type` routing (unmatched types fall back to the orchestrator runner — see ADR-030) | `extensions/queue-ingress/__tests__/item-pipeline.test.ts`, `__tests__/production-wiring.test.ts`, `test/src/e2e-item-pipeline.test.ts` |
| **Cost control** (`budget/v1`) | Hard per-item `max_cost_usd` (item halts and dead-letters `BUDGET_EXCEEDED`, never the queue) and a per-UTC-day budget that pauses consumption — both wired in the production consumer | `extensions/queue-ingress/__tests__/cost-control.test.ts`, `__tests__/production-wiring.test.ts` |
...
| **Escalation bridge** (`escalation/v1`) | A failing item escalates into a signed envelope, runs in Flow, and closes on a signed resolution under one correlation id — the emitter is wired in the production consumer, armed by `FORGE_INTAKE_URL` + `FORGE_BRIDGE_SECRET` (unarmed, escalations dead-letter `ESCALATION_UNARMED`; see ADR-033) | `extensions/queue-ingress/__tests__/escalation.test.ts`, `__tests__/production-wiring.test.ts`, `test/src/bridge-e2e.test.ts` |
